...
public class booking extends HttpServlet {
   ...
   protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    System.out.println("controller.bookTicket.booking.processRequest()");

    try {
        c_id = dao.getEid(email, pass, flag);
    } catch (Exception e) {
        request.setAttribute("loginerror", "Lỗi khi truy vấn cơ sở dữ liệu!");
    ...
    if (from != null && to != null && hourStr != null && minuteStr != null) {
        try {
            ...
            if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
                ...
                trips = dao.getBusTrips(from, to, time, travelDate);

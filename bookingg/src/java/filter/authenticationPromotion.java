 ...
 * @author ADMIN
 */
public class authenticationPromotion implements Filter {
    ...
    public authenticationPromotion() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("authenticationPromotion:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("authenticationPromotion:DoAfterProcessing");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (path.startsWith("/promotion")) {
            ...
            if (sessionUser instanceof User) {
                ...
                if (user.getRoleID() == 5) { // 1 là RoleID của Blog Manager
                    chain.doFilter(request, response);
                    return;
                } else {
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (debug) {                
                log("authenticationPromotion:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("authenticationPromotion()");
        }
        StringBuffer sb = new StringBuffer("authenticationPromotion(");

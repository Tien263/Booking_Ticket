 ...
 * @author ADMIN
 */
public class authenticationVehicle implements Filter {
    ...
    public authenticationVehicle() {
    }    
    
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("authenticationVehicle:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("authenticationVehicle:DoAfterProcessing");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (path.startsWith("/vehicle")) {
            ...
            if (sessionUser instanceof User) {
                ...
                if (user.getRoleID() == 6) { // 1 là RoleID của Blog Manager
                    chain.doFilter(request, response);
                    return;
                } else {
                    httpResponse.sendRedirect(httpRequest.getContextPath() + "/employee_login.jsp?RoleErr=true");
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (debug) {                
                log("authenticationVehicle:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("authenticationVehicle()");
        }
        StringBuffer sb = new StringBuffer("authenticationVehicle(");
